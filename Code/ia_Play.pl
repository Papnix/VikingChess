...
checkPlaceIsSafe(X, Y, [[Xbis, Ybis|_]|List]):-
	currentPlayer(Player),Player = 'A',
	getCaseOnBoard(Xbis, Ybis, Pawn), 
	not(Pawn = '_D_'), not(Pawn = '_R_'), not(Pawn = '_X_'),
	checkPlaceIsSafe(X, Y, List).
												
checkPlaceIsSafe(X, Y, [[Xbis, Ybis|_]|List]):-
	currentPlayer(Player),Player = 'A',
	getCaseOnBoard(Xbis, Ybis, Pawn),
	((Pawn = '_D_');(Pawn = '_R_');(Pawn = '_X_')),
	(Xop is 2*X-Xbis, Yop is 2*Y-Ybis, getOtherPiece(PieceList), (pieceOp2(Pawn, Xop, Yop, PieceList);(!,fail))),
	checkPlaceIsSafe(X, Y, List).

checkPlaceIsSafe(X, Y, [[Xbis, Ybis|_]|List]):-
	currentPlayer(Player), Player = 'D',
	getCaseOnBoard(Xbis, Ybis, Pawn),
	not(Pawn = '_A_'), not(Pawn = '_X_'),
	checkPlaceIsSafe(X, Y, List).
...
checkPlaceIsSafe(X, Y, [[Xbis, Ybis|_]|List]):-
	...
	getCaseOnBoard(Xbis, Ybis, Pawn),
	Pawn = '_A_', Pawn = '_X_',
	Xop is 2*X-Xbis, Yop is 2*Y-Ybis,
	getOtherPiece(PieceList),
	(pieceOp2(Pawn, Xop, Yop, PieceList);(!,fail)),
	checkPlaceIsSafe(X, Y, List).

checkPlaceIsSafe(X, Y, [[Xbis, Ybis|_]|List]):- getCaseOnBoard(Xbis, Ybis, _) ; checkPlaceIsSafe(X,Y,List).
...
pieceOp2(_,_,_,[]):-!.
pieceOp2(Pawn, X, Y, [[X, Ybis|_]|List]):- (not(reachableY(X,Ybis,Y,Pawn));!,fail), pieceOp2(Pawn,X,Y,List).
pieceOp2(Pawn, X, Y, [[Xbis, Y|_]|List]):- (not(reachableX(Xbis,Y,X,Pawn));!,fail), pieceOp2(Pawn,X,Y,List).
pieceOp2(Pawn, X, Y, [[_, _|_]|List]):- pieceOp2(Pawn,X,Y,List).
